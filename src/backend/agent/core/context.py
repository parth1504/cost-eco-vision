...
an LLM directly — this layer curates the context window with:
  ...
  - Cross-resource correlations (from the correlation agent)
...
def _slim_resource(resource: Dict[str, Any]) -> Dict[str, Any]:
    """Strip large/irrelevant fields from resource before sending to agent."""
    keep_keys = {
...
def _is_relevant_recommendation(agent_id: str, rec: Dict[str, Any]) -> bool:
    ...
    mapping = {
        "ec2_specialist": {"cost", "performance", "reliability", "security", "operational"},
        "s3_specialist": {"cost", "performance", "reliability", "security", "operational"},
        "dynamodb_specialist": {"cost", "performance", "reliability", "security", "operational"},
        "critique": set(),
        "correlation": set(),
    }
    allowed = mapping.get(agent_id)
    if allowed is None:
        return True
...
def build_critique_context(
    ...
    """Build context for the critique agent to review recommendations."""
    return {
        "agent_id": "critique",
        "resource": _slim_resource(resource),
...
def build_correlation_context(
    ...
    """Build context for the correlation agent to find cross-resource patterns."""
    return {
